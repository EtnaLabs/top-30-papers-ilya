...
// Direct copy of the data we need
const items = [
  {
    type: "paper",
    order: 1,
...
// Export as JSON
function exportAsJSON(outputPath) {
...
// Export as CSV
function exportAsCSV(outputPath) {
  // Create CSV header with separate slide columns
  const headers = ['order', 'date', 'title', 'authors', 'link', 'summary', 'takeaways', 'slide3', 'slide4'];
  ...
  items.forEach(paper => {
    ...
    // Process up to 4 slides
    for (let i = 0; i < 4; i++) {
      if (paper.slides && paper.slides[i]) {
        const slide = paper.slides[i];
        let slideContent = "";
        
        // Extract content based on what's available in the slide
        if (slide.title) {
        ...
        if (slide.bullets && slide.bullets.length > 0) {
          if (slideContent) slideContent += " | ";
          slideContent += `Bullets: ${slide.bullets.join(" | ")}`;
        }
        ...
        slideFields.push(`"${slideContent.replace(/"/g, '""')}"`);
      } else {
        // Empty slide
        slideFields.push('""');
      }
